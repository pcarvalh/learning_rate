...
dataset_path <- "ds99_student_step_All_Data_211_2016_0423_082151.txt"
dataset <- strsplit(dataset_path,"_")[[1]][1]

kcm <- 'KC (LFA_AIC_LIB_Model0)'
response <- 'First Attempt'
opportunity <- 'Opportunity (LFA_AIC_LIB_Model0)'
individual <- 'Anon Student Id'
time <- 'First Transaction Time'
prediction <- 'Predicted Error Rate (LFA_AIC_LIB_Model0)'
...
irafm.model <- suppressWarnings(glmer(success ~  (1|individual) (timeDiff|individual) + (timeDiff|KC) - 1, data=df_ordered, family=binomial(),control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))))
```

...
sum_df <- df_ordered[,.(actual_errorRate = mean(success),predicted_errorRate = mean(prediction)),by=time_opp]
...
maineffect_intercept
maineffect_slope <- fixef(irafm.model)[[2]]
maineffect_slope

overall.params <- data.frame(cbind(model_AIC,model_BIC,model_logLik,maineffect_intercept,maineffect_slope))
overall.params

fwrite(overall.params,file = paste("params/","overall.params_",dataset,".csv",sep=""))
...
stud.params
fwrite(stud.params,file = paste("params/","stud.params_",dataset,".csv",sep=""))
...
kc.params
fwrite(kc.params,file = paste("params/","kc.params_",dataset,".csv",sep=""))

...
dataset_path <- "ds562_student_step_All_Data_1788_2016_0413_074846.txt"
dataset <- strsplit(dataset_path,"_")[[1]][1]

kcm <- 'KC (new_supportType_topic)'
response <- 'First Attempt'
opportunity <- 'Opportunity (new_supportType_topic)'
individual <- 'Anon Student Id'
time <- 'First Transaction Time'
prediction <- 'Predicted Error Rate (new_supportType_topic)'
...
ggplot(data=sum_df_long, aes(x=time_opp, y=ErrorRate,shape = typeRate,colour = typeRate)) +
  geom_point() + geom_line() + xlim(0,80)
```

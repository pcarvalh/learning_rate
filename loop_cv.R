...
for(dataset in 1:nrow(dataset_list)){
  ...
  tryCatch(
    {
    #function(inputFile,folds,repeats,inputFile,kcm,response,opportunity,individual)
      cv_iafm <- cv(modelToFit= "tAFM",inputFile = dataset_list$StudentStep[dataset],folds=10,repeats=5,kcm = dataset_list$KC[dataset],response = "First Attempt",opportunity = dataset_list$Opportunity[dataset],individual = "Anon Student Id")

...
library(data.table)
df <- fread("data/ds73_student_step_All_Data_62_2016_0401_112846.txt")
```
...
opportunity <- 'Opportunity (tones-h-syl)'
individual <- 'Anon Student Id'
time <- 'First Transaction Time'
prediction <- 'Predicted Error Rate (tones-h-syl)'
...
df_ordered$log_timeDiff <- log(as.numeric(df_ordered$timeDiff))
df_ordered$log_timeDiff <- ifelse(df_ordered$log_timeDiff==-Inf,NA,df_ordered$log_timeDiff)
...
library(lme4)
library(optimx)
irafm.model <- suppressWarnings(glmer(errorRate ~ log_timeDiff + (log_timeDiff|individual) + (log_timeDiff|KC), data=df_ordered, family=binomial(),control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))))
```
...
df_ordered$log_timeDiff <- round(df_ordered$log_timeDiff)
sum_df <- df_ordered[,.(actual_errorRate = mean(errorRate),predicted_errorRate = mean(prediction)),by=log_timeDiff]

sum_df_long <- melt(data = sum_df,id.vars = c("log_timeDiff"),measure.vars = c("actual_errorRate","predicted_errorRate"),variable.name = "typeRate",value.name = "ErrorRate")

ggplot(data=sum_df_long, aes(x=log_timeDiff, y=ErrorRate,shape = typeRate,colour = typeRate)) +
